...
__device__ real d_forcingTerm(unsigned int i, unsigned int j, real delta_x, real t)
{
    ...
    return ((d_chi*d_Cm*d_pi*cos(d_pi*t)) + (2*d_pi*d_pi*d_sigma*sin(d_pi*t)/(d_L*d_L)) + (0*d_G*sin(d_pi*t))) * sin(d_pi*x/d_L) * sin(d_pi*y/d_L);
}
...
__global__ void parallelRHSForcing_theta(real *d_V, real *d_Rv, unsigned int N, real t, real delta_t, real delta_x, real phi, real theta, int discFibxMax, int discFibxMin, int discFibyMax, int discFibyMin, real fibrosisFactor)
{
    ...
    if (index < N*N)
    {
        ...
        real actualRHS_V = (1.0 / (d_Cm * d_chi)) * ((-d_G*actualV*0) + d_forcingTerm(i, j, delta_x, t));
        
        Vtilde = actualV + (delta_t * actualRHS_V) + (phi * diffusion);
        // Vtilde = actualV + (theta * delta_t * actualRHS_V) + (theta * phi * diffusion); // promising

        real tildeRHS_V = (1.0 / (d_Cm * d_chi)) * ((-d_G*Vtilde*0) + d_forcingTerm(i, j, delta_x, t+(1.0*delta_t)));     // alpha = 1.0 (Heun's) / 0.5 (midpoint) / 2/3 (Ralston's)
...
__global__ void parallelRHSForcing_SSI(real *d_V, real *d_Rv, unsigned int N, real t, real delta_t, real delta_x, real phi, real theta, int discFibxMax, int discFibxMin, int discFibyMax, int discFibyMin, real fibrosisFactor)
{
    ...
    if (index < N*N)
    {
        ...
        real tildeRHS_V = (1.0 / (d_Cm * d_chi)) * ((-d_G*Vtilde*0) + d_forcingTerm(i, j, delta_x, t+(0.5*delta_t)));     // promising
